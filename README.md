...
3. Copy the environment templates (choose the command for your OS).

   - **Windows PowerShell**

     ```powershell
     Copy-Item ..\.env.example ..\.env.local         # Optional root-level config
...
4. Configure secrets.
   ...
   - Edit `diamondz-playhouse/backend/.env.local` with your MongoDB URI and Stripe secrets.
   - Edit `diamondz-playhouse/frontend/.env.local` with the API base URL and Stripe publishable key.
...
**Backend (`diamondz-playhouse/backend/.env.local`):**
...
**Frontend (`diamondz-playhouse/frontend/.env.local`):**
...
│   ├── frontend/                # React app
│   ├── backend/                 # Node.js API
│   │   └── secure-vault/        # 🔒 Paid content (after purchase)
│   │       ├── Comics/          # PDF downloads
│   │       ├── Puzzles/         # Word searches, jigsaws, etc.
│   │       └── Wallpapers/      # 4K high-res art
│   ├── docs/                    # Documentation
│   └── assets/                  # Public graphics
└── Stories/                     # Raw creative files
   ├── Comics/
   ├── PimpinPaul/
   ├── DiamondHeist/
   └── EyeOfPempire/
